 ...
 * @LastEditTime: 2019-05-27 15:04:06
 */
...
export default function useBigDataRender({
  ...
  const [rangeHeightList, setRangeHeightList] = useState(
    new Array(Math.ceil(data.length / range))
  );
  // 超速滚动
  ...
  const handleScroll = function(e) {
    ...
    if (speed > 20) {
      ...
      setTimer(
        setTimeout(() => {
          ...
          setOverSpeed(false);
          const [nextStep, nextOffsetTop] = getIndexAndOffsetTopByScrollTop(
            scrollTop
          );
          console.table({
            nextStep,
    ...
    if (scrollTop > prevScrollTop) {
      ...
      const curDistance =
        offsetTop + contentRef.current.clientHeight - scrollTop - height;
   ...
   */
  const setStepAndOffsetTop = useCallback((nextStep, nextOffsetTop) => {
    // 顶部
    if (nextStep <= 0) {
      setStep(0);
      setOffsetTop(0);
  ...
  const getOffsetTopByIndex = useCallback(function(nextStep) {
    let newOffsetTop = 0;
   ...
   */
  const getIndexAndOffsetTopByScrollTop = useCallback(function(scrollTop) {
    let totalHeight = 0;
   ...
   * 计算
   */
  const computeRangeAndTotalHeight = useCallback(function() {
    ...
    // 节点总高度
    const newRangeHeight = nodeList.reduce(
      (prev, cur) => prev + cur.clientHeight,
      0
    );
    //
    const firstRangeHeight = rangeHeightListCompleted
      ? rangeHeightList[0]
      : newRangeHeight;
  ...
  }, [data.length, range, shouldRenderDirectly, step, timer]);
  ...
  }, [computeRangeAndTotalHeight, data.length]);
  ...
  useEffect(() => {
    ...
    const scrollTop =
      totalHeight < prevScrollTop
        ? totalHeight - rangeHeightList[0] * 2
        : prevScrollTop;
    const [nextStep, nextOffsetTop] = getIndexAndOffsetTopByScrollTop(
      scrollTop
    );
    ...
    getIndexAndOffsetTopByScrollTop,
    prevScrollTop,
    rangeHeightList,
  ...
  useEffect(() => {
    ...
    const { scrollTop } = contentEl.parentElement.parentElement;
  ...
  return {
    ...
    data: shouldRenderDirectly
      ? data
      : data.slice(startIndex, (step + 2) * range),
  };
}
