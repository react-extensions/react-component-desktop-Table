...
class Checkbox extends PureComponent {
  ...
  render() {
    ...
    const { isControlled, checked: stateChecked } = this.state;

    const checked = isControlled ? propsChecked : stateChecked;
    const Icon = checked
      ? checkedIcon || config.icon.Check
      : notCheckedIcon || config.icon.NotCheck;

    return (
      <label
        className={`r-checkbox-wrapper ${className || ''}${
          disabled ? ' _disabled' : ''
        }`}
        {...rest}
